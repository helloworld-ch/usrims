<template>
  <div id = "student">
    ...
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item>
        <el-input v-model="teacherQuery.id" placeholder="学号"/>
      </el-form-item>
      <el-form-item>
        <el-input v-model="teacherQuery.name" placeholder="学生名"/>
      </el-form-item>
      <el-form-item>
        <el-select v-model="teacherQuery.grade" clearable placeholder="年级">
    ...
    <el-table
      ...
      <el-table-column prop="grade" label="年级" width="180" align="center">
        <template slot-scope="scope">
          {{ scope.row.grade===1?'大一':scope.row.grade===2?'大二':
            scope.row.grade===3?'大三':'大四' }}
        </template>
      </el-table-column>

      <el-table-column prop="collage" label="学校" width="220" align="center"/>

      <el-table-column prop="number" label="参与研究数目" width="150" align="center"/>
...
export default {
  ...
  methods:{
    getList(page=1) {
      this.page = page
      student.getTeacherListPage(this.page,this.limit,this.teacherQuery)
          .then(response =>{//请求成功
